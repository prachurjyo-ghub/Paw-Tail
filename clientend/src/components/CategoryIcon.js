...
function SmallPetIcon({ className = "w-8 h-8", strokeWidth = 1.8, style }) {
  return (
    <svg
      viewBox="0 0 24 24"
      fill="none"
...
function CamelliaIcon({ className = "w-8 h-8", strokeWidth = 1.8, style }) {
  return (
    <svg
      viewBox="0 0 24 24"
      fill="none"
    ...
    >
      ...
      <path d="M12 12c0-3 2-5 4-6" />
      <path d="M12 12c0-3-2-5-4-6" />
      <path d="M12 12c-2 2-2 5 0 7" />
      <path d="M12 12c2 2 2 5 0 7" />
...
export default function CategoryIcon({
  ...
  // We explicitly set inline width/height to 100% so it perfectly fills its parent container
  // and ignores any hydration mismatch from size=24 defaults.
  return (
    <div className={`flex items-center justify-center ${className}`}>
      <IconComponent style={{ width: "100%", height: "100%" }} strokeWidth={strokeWidth} />
    </div>
  );
}
